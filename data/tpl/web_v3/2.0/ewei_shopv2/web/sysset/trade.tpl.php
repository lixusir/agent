...
<!-- 添加优化 自提和到付订单含运费显示 -->
<div class="form-group-title">自提和到付订单运费显示</div>
    <div class="form-group">
        <label class="col-lg control-label">用户订单详情页是否显示“（含运费）”</label>
        <div class="col-sm-9 col-xs-12">
            <label class='radio-inline'><input type='radio' class="freight" name='data[freight]' value='1' <?php  if($data['freight']==1) { ?>checked<?php  } ?>/> 是</label>
            <label class='radio-inline'><input type='radio' class="freight" name='data[freight]' value='0' <?php  if($data['freight']==0) { ?>checked<?php  } ?>/> 否</label>
        </div>
    </div>
    <div class="form-group recharge-group freightText">
        <label class="col-lg control-label">自定义字段</label>
        <div class="col-sm-9 col-xs-12">
            <div class='input-group fixmore-input-group'>
                <span class='input-group-addon'>含运费=</span>
                <input type="text" name="data[freightText]" class="form-control" value="<?php  echo $data['freightText'];?>" />
            </div>
            <span class='help-block'>如不填写则默认为系统当前字段</span>
        </div>
    </div>
    <div class="form-group recharge-group">
        <label class="col-lg control-label">订单运费显示</label>
        <div class="col-sm-9 col-xs-12">
            <label class='radio-inline'><input type='radio'  name='data[freightShow]' value='0' <?php  if($data['freightShow']==0) { ?>checked<?php  } ?>/> 是</label>
            <label class='radio-inline'><input type='radio'  name='data[freightShow]' value='1' <?php  if($data['freightShow']==1) { ?>checked<?php  } ?>/> 否</label>
            <span class='help-block'>隐藏条件为订单包邮或运费为0；作用于自提订单的确认订单页面、订单详情页面以及快递配送货到付款订单的详情页面</span>
        </div>
    </div>
    ...
    <div class='form-group-title'>核销时间计算方式</div>
    ...
    <div class='panel-body'>
        ...
        <div class="form-group">
            <label class="col-lg control-label">核销订单计算方式</label>
            <div class="col-sm-9 col-xs-12">
                <?php if(cv('sysset.trade.edit')) { ?>
                <label class='radio-inline'><input type='radio' name='data[verify_type]' value='0' <?php  if($data['verify_type']==0) { ?>checked<?php  } ?> />下单时间 </label>
                <label class='radio-inline'><input type='radio' name='data[verify_type]' value='1' <?php  if($data['verify_type']==1) { ?>checked<?php  } ?>/> 核销时间</label>
                <span class='help-block'>商城已产生的核销数据默认为下单时间,修改后历史数据不兼容,</span>
                <?php  } else { ?>
                <input type="hidden" name="data[verify_type]" value="<?php  echo $data['verify_type'];?>"/>
                <div class='form-control-static'><?php  if($data['verify_type']==1) { ?>下单时间<?php  } else { ?>核销时间<?php  } ?></div>
                <?php  } ?>
            </div>
        </div>
    </div>
...
<script>
    if($('input[name="data[freight]"]:checked').val() == 1) {
        $('.freightText').show()
    }else {
        $('.freightText').hide()
    }
